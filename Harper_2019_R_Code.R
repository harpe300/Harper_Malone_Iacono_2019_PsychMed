...
# Plot mean and 95% CI for each assessment, split by group, layered on a spaghetti plot
ggplot(tgc, aes(age, CSU_drink_index,group=Group))+  geom_point(data = melted, aes(x=age, y = CSU_drink_index, group = 1),alpha=.5) +geom_errorbar(aes(ymin=CSU_drink_index-ci, ymax=CSU_drink_index+ci), width=.075, size=1.5) + geom_line(aes(color=Group),size = 2) + geom_point(aes(color=Group),size=3) + theme_classic() + scale_color_manual(values= cbPalette) + labs(x = "Assessment Age", y = "Score", title = "Drink Index by Age and Group") + scale_y_continuous(breaks=c(0:17)) +  geom_path(data = melted, aes(x=age, y = CSU_drink_index, group = ID),alpha = .3)
...
# plot mixed effect model partial residuals to visualize effects
...
# Visualize partial residuals to depict effects and check for outliers
visreg::visreg(P3Theta_LMM.model,"TDp3mPZPOZT", overlay=TRUE, line=list(col=c("red3")),fill=list(col=c("#000000")),points=list(col=c("#000000"), pch=c(1),size=1.5), xlab="P3 amplitude scores", ylab="Age-17 Drink Index scores", gg=TRUE) + ggplot2::theme_classic() + ggplot2::ggtitle( 'LMM Partial Residual Plot')
visreg::visreg(P3Theta_LMM.model,"TPROIthmFCZT", overlay=TRUE, line=list(col=c("red3")),fill=list(col=c("#000000")),points=list(col=c("#000000"), pch=c(1),size=1.5), xlab="Theta power scores", ylab="Age-17 Drink Index scores", gg=TRUE) + ggplot2::theme_classic() + ggplot2::ggtitle( 'LMM Partial Residual Plot')
}
